...
public class ChartQueryController {
  ...
  public HttpApiResponse<ChartResponse.ChartData> getStockCharts(@PathVariable String market,
    ...
    List<ChartResponse.ChartItem> chartItems = stockChartData.getChartData().stream()
        .map(item -> new ChartResponse.ChartItem(item.getDate(), item.getOpen(), item.getHigh(), item.getLow(),
