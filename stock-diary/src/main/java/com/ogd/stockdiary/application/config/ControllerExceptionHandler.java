...
public class ControllerExceptionHandler {
  ...
  public ResponseEntity handleApplicationException(ApplicationException e) {
    ...
    if (e.getCode() == CodeEnum.FRS_002) {
      return ResponseEntity.status(HttpStatus.FORBIDDEN).body(HttpApiResponse.fromExceptionMessage(
          e.getMessage() != null ? e.getMessage() : e.getCode().getDescription(), e.getCode(), e.getData()));
    }

    return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(HttpApiResponse.fromExceptionMessage(
        e.getMessage() != null ? e.getMessage() : e.getCode().getDescription(), e.getCode(), e.getData()));
  }
  ...
  public ResponseEntity<HttpApiResponse<?>> handleMethodNotSupported(HttpRequestMethodNotSupportedException e) {
    ...
    return ResponseEntity.status(HttpStatus.METHOD_NOT_ALLOWED)
        .body(HttpApiResponse.fromExceptionMessage(CodeEnum.FRS_005.getDescription(), CodeEnum.FRS_005, null));
  }
  ...
  public ResponseEntity<HttpApiResponse<?>> handleMissingServletRequestParameterException(
    ...
    logger.error("MissingServletRequestParameter Exception occurred. parameterName={}, message={}",
        e.getParameterName(), e.getMessage(), e);
    return ResponseEntity.status(HttpStatus.BAD_REQUEST)
  ...
  private String createMessage(MethodArgumentNotValidException e) {
    ...
    if (fe != null && fe.getDefaultMessage() != null) {
      return fe.getDefaultMessage() != null ? fe.getDefaultMessage() : "알수없는 오류가 발생했습니다.";
    }
  ...
  public ResponseEntity handleException(Exception e) {
    ...
    return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
        .body(HttpApiResponse.fromExceptionMessage(CodeEnum.FRS_004, CodeEnum.FRS_004.getDescription()));
  }
}
