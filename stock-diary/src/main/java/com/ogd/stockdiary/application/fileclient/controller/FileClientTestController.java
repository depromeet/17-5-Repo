 ...
 * 파일 클라이언트 테스트 컨트롤러
 *
 * <p><b>⚠️ 테스트 전용:</b> 이 컨트롤러는 MinIO 파일 업로드/다운로드 기능을 테스트하기 위한 용도입니다.
 *
 * <p><b>🔴 추후 삭제 예정:</b> 프로덕션 배포 전 반드시 제거해야 합니다.
 *
 * <p><b>구현:</b> {@link com.ogd.stockdiary.domain.fileclient.port.out.FileClientPort}를 사용합니다.
 */
...
public class FileClientTestController {
    ...
    @Operation(
            summary = "[TEST] 업로드 URL 생성",
            description = "파일 업로드를 위한 Pre-Signed URL을 생성합니다. " + "⚠️ 테스트 전용 - 추후 삭제 예정")
    ...
    public ResponseEntity<HttpApiResponse<PreSignedUrlResponse>> createUploadUrl(
        ...
        String url =
                fileClientTestUseCase.createUploadUrl(request.getObjectKey(), request.getTtl());

        PreSignedUrlResponse response =
                new PreSignedUrlResponse(url, request.getObjectKey(), request.getTtl());

        return ResponseEntity.ok(HttpApiResponse.of(response));
    }

    @Operation(
            summary = "[TEST] 파일 직접 업로드",
            description = "파일을 MinIO에 직접 업로드합니다. " + "⚠️ 테스트 전용 - 추후 삭제 예정")
    ...
    public ResponseEntity<HttpApiResponse<FileUploadResponse>> uploadFile(
        ...
        FileUploadResponse response =
                new FileUploadResponse(objectKey, file.getSize(), "파일 업로드 성공");

        return ResponseEntity.ok(HttpApiResponse.of(response));
    }

    @Operation(
            summary = "[TEST] 다운로드 URL 생성",
            description = "파일 다운로드를 위한 Pre-Signed URL을 생성합니다. " + "⚠️ 테스트 전용 - 추후 삭제 예정")
