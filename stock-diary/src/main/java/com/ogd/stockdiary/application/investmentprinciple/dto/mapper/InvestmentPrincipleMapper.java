...
public class InvestmentPrincipleMapper {
  ...
  public static CreateMultiplePrinciplesCommand toCommand(CreateMultiplePrinciplesRequest request, Long userId) {
    return new CreateMultiplePrinciplesCommand(userId, request.getPrinciples());
  }

  public static UpdatePrincipleCommand toCommand(UpdatePrincipleRequest request, Long principleId, Long userId) {
  ...
  public static BatchProcessCommand toCommand(BatchProcessRequest request, Long userId) {
    ...
    if (request.getUpdatePrinciples() != null) {
      updateCommands = request.getUpdatePrinciples().stream()
          .map(update -> new UpdatePrincipleCommand(update.getPrincipleId(), userId, update.getPrinciple()))
          .collect(Collectors.toList());
    }

    return new BatchProcessCommand(userId, request.getCreatePrinciples(), updateCommands,
        request.getDeletePrincipleIds());
  }

  public static InvestmentPrincipleResponse toResponse(InvestmentPrinciple principle) {
    return new InvestmentPrincipleResponse(principle.getId(), principle.getPrinciple(), principle.getCreatedAt(),
        principle.getUpdatedAt());
  }

  public static List<InvestmentPrincipleResponse> toResponseList(List<InvestmentPrinciple> principles) {
    return principles.stream().map(InvestmentPrincipleMapper::toResponse).collect(Collectors.toList());
  }

  public static BatchProcessResponse toBatchResponse(BatchProcessResult result) {
    return new BatchProcessResponse(toResponseList(result.getCreatedPrinciples()),
        toResponseList(result.getUpdatedPrinciples()), result.getDeletedPrincipleIds());
  }
}
