...
public class AppleOAuthClient implements OAuthClient {
  ...
  public OAuthTokenResponse getToken(String authCode) {
    ...
    return restClient.post().uri(APPLE_AUTH_URL + TOKEN_ENDPOINT).contentType(MediaType.APPLICATION_FORM_URLENCODED)
        .body(params).retrieve().body(OAuthTokenResponse.class);
  }

  @Override
  @Cacheable(value = "oidcPublicKeys", key = "'apple'")
  public OIDCPublicKeyList getPublicKeys() {
    return restClient.get().uri(APPLE_AUTH_URL + KEYS_ENDPOINT).retrieve().body(OIDCPublicKeyList.class);
  }
  ...
  public void unlink(String identifier) {
    ...
    restClient.post().uri(APPLE_AUTH_URL + REVOKE_ENDPOINT).contentType(MediaType.APPLICATION_FORM_URLENCODED)
        .body(params).retrieve().toBodilessEntity();
  }
  ...
  private String generateClientSecret() {
    try {
      ...
      return Jwts.builder().setHeaderParam("kid", appleProperties.getKeyId()).setHeaderParam("alg", "ES256")
          .setIssuer(appleProperties.getTeamId()).setIssuedAt(issuedAt).setExpiration(expiration)
          .setAudience(appleProperties.getAud()).setSubject(appleProperties.getClientId())
