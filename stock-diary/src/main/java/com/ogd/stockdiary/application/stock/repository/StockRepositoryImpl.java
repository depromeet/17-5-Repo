...
public class StockRepositoryImpl implements StockRepository {
    ...
    @Override
    public SliceContent<Stock> findByCompanyNameSlice(String nextCursor, String companyName, int size) {
        List<Stock> content;

        if (nextCursor == null) {
            content = jpaStockRepository.findByCompanyNameLikeOrderByIdDesc(companyName, PageRequest.of(0, size));
        } else {
            content = jpaStockRepository.findByCompanyNameLikeOrderByIdDesc(companyName, Integer.parseInt(nextCursor), PageRequest.of(0, size));
        }
