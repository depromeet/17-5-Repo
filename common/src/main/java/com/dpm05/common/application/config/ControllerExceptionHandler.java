package com.dpm05.common.application.config;

import com.dpm05.common.common.httpresponse.CodeEnum;
import com.dpm05.common.common.httpresponse.HttpApiResponse;
import com.dpm05.common.exception.ApplicationException;
...
public class ControllerExceptionHandler {
        ...
        if (e.getCode() == CodeEnum.FRS_002) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN)
                    .body(HttpApiResponse.fromExceptionMessage(
                            e.getMessage() != null ? e.getMessage() : e.getCode().getDescription(),
                            e.getCode(),
                            e.getData()
                    ));
        }
    ...
    public ResponseEntity<HttpApiResponse<?>> handleMissingServletRequestParameterException(MissingServletRequestParameterException e) {
        logger.error("MissingServletRequestParameter Exception occurred. parameterName={}, message={}", e.getParameterName(), e.getMessage(), e);
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(HttpApiResponse.fromExceptionMessage(
                        CodeEnum.FRS_003,
                        e.getMessage()
